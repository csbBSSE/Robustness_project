...
for i in params['file_reqs']:
    for j in params['input_filenames']:
                ...
                prob_arr = []
                filename_index = 0
                act_file = open("{}_{}activation.txt".format(j,version), "w")
                ...
                #bool_sum=0
                #for q in range(len(prob_arr)):
                #        bool_sum+=prob_arr[q]
                #prob_arr=np.array(prob_arr) / bool_sum   

              
                rcParams.update({'figure.autolayout':True})    
                #plt.figure(figsize = (20,10))           
                fig, ax = plt.subplots(1,1)
                plt.scatter(prob_arr, activation_arr,s=8)
                plt.tight_layout()
                
                plt.savefig("Activationgraphs/{}_{}activationplot.png".format(j,version))
                plt.clf()
                           
                rac_probfile = open("Datafiles/{}_racipe_probfull.txt".format(j, "r"))
                ...
                prob_arr = []
                filename_index = 0
                act_file = open("{}_racactivation.txt".format(j+i), "w")
                act_file.write("State ActivationSum Probability\n")
                for p in range(len(params['input_filenames'])):
                    if j == params['input_filenames'][p]:
                        filename_index = p
                ...
                plt.scatter(prob_arr, activation_arr,s=8)
                plt.tight_layout()
                plt.savefig("Activationgraphs/{}_racactivationplot.png".format(j))
                plt.clf()
