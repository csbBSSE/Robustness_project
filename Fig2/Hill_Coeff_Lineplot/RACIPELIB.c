...
void check_inputfile (int argc, char **argv, struct topo *topoinfo, struct opts *simu_opts, struct rlt *tmprlt)
{
  ...
  char temptopofile[500];
  strcpy(temptopofile, "input/");
  strcat(temptopofile, argv[1]);
  
  char tempidsfile[500];
  strcpy(tempidsfile, "input/");
  strcat(tempidsfile, argv[2]);
  ...
  if (strcmp(fileextension, "topo") == 0) {
    ...
    Model_generate1   (temptopofile, tempidsfile, topoinfo, simu_opts, tmprlt);
  }
  else if ((strcmp(fileextension, "cfg") == 0)) {
    simu_opts->exts = 1; // flag for input file as cfg file
    read_cfg         (topoinfo, simu_opts, tmprlt);
    initial_simuopts (argc, argv, topoinfo, simu_opts);
    Model_generate   (temptopofile, topoinfo, simu_opts, tmprlt);
  }
  else {
  ...
  switch (dist) {
    ...
    case 2 :
      ...
      for(i = 0; i < topoinfo->numG; i++){
        ...
        //estimate_threshold(int num, int ID, double minP, double maxP, double minK, double maxK, double minN, double maxN, double minF, double maxF, double *minT, double *maxT, struct topo *topoinfo, int dist, double SF)
        // printf("%s = %f\n", topoinfo->Gname[i], (minT+maxT)/2.0);
...
void solve_ODE_euler (int j, struct opts *simu_opts, struct topo *topoinfo, struct rlt *tmprlt)
{
  ...
  // printf("%f %f %f %f: %f %f %f %f\n", y[0], y[1], y[2], y[3], ytmp[0], ytmp[1], ytmp[2], ytmp[3]);
